...
def import_data(pth):
    ...
    Returns:
        churn_data: (pandas.core.frame.DataFrame) a pandas dataframe with the 'churn' column.
...
def perform_eda(churn_data):
    '''
    Perform exploratory data analysis on 'churn_data' and save figures to images folder.

    Args:
        churn_data: (pandas.core.frame.DataFrame) pandas dataframe with the 'churn' column.
    ...
    churn_data['churn'].hist()
    plt.savefig('./images/eda/churn_distribution')

    plt.figure(figsize=(20, 10))
    churn_data['Customer_Age'].hist()
    plt.savefig('./images/eda/customer_age_distribution')

    plt.figure(figsize=(20, 10))
    churn_data['Marital_Status'].value_counts('normalize').plot(kind='bar')
    plt.savefig('./images/eda/marital_status_distribution')

    plt.figure(figsize=(20, 10))
    sns.histplot(churn_data['Total_Trans_Ct'], kde=True)
...
def _find_category_churn(churn_dataframe, category_name, response):
    '''
    ...
    churn_info = churn_dataframe.groupby(category_name)[response].mean()
    result = dict(zip(churn_info.index, churn_info.values))

    return result


def encoder_helper(churn_data, category_lst, response):
    '''
    ...
    for category in category_lst:
        mapping_information = _find_category_churn(churn_data, category, response)
        churn_data[category + '_churn'] = churn_data[category].map(mapping_information)
...
def perform_feature_engineering(churn_data, response):
    ...
    Args:
        churn_data: (pandas.core.frame.DataFrame) pandas dataframe
    ...
    dataframe_encoded = encoder_helper(churn_data,
                                       ['Gender',
                                        'Education_Level',
                                        'Marital_Status',
                                        'Income_Category',
...
def train_models(X_train, X_test, y_train, y_test):
    ...
    # ROC AUC
    roc_figure, figure_axes = plt.subplots(figsize=(15, 8))
    plot_roc_curve(grid_random_forest.best_estimator_,
                   X_test, y_test, ax=figure_axes, alpha=0.8)
    plot_roc_curve(logistic_regression, X_test, y_test, ax=figure_axes, alpha=0.8)
    roc_figure.savefig('./images/results/roc_curve_result')
